...
namespace lab7_cs{
    class Program{
        static void Main(string[] args){
            ...
            double[] y = new double[]{1, 5, 3, 5, 2, 3};
    ...
    public class TridiagMatrix {
	    ...
	    public double sine(double e){
		    ...
		    if (Q>1) answer *= -1;
		    //Console.Write(String.Format("{0:0.########}{0:+.########}(x{0:+.########}){0:+.########}(x{0:+.########})^2{0:+.########}(x{0:+.########})^3\n", y[idx], o1[idx], x[idx]*-1, o2[idx], x[idx]*-1, o3[idx], x[idx]*-1);
		    return answer;
	    }
	    ...
	    private void solveMatrix(){
            ...
            for (int idx = 0; idx < n; idx++) {
                //Console.Write(String.Format("{0:0.########}{1:+.########}(x{2:+.########}){3:+.########}(x{4:+.########})^2{5:+.########}(x{6:+.########})^3\n", y[idx], o1[idx], x[idx]*-1, o2[idx], x[idx]*-1, o3[idx], x[idx]*-1));
                Console.WriteLine(String.Format("{0}: {1} {2} {3} {4}", idx, y[idx], o1[idx], o2[idx], o3[idx]));
            }
        }
